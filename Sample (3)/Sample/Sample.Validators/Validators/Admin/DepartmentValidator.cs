...
namespace Sample.Validator.Validators.Admin
{
    ...
    {
        readonly IDepartmentService _userService;
        public DepartmentValidator(IDepartmentService userService)
        {
            _userService = userService;
        }


        public async Task<List<(int, string)>> ValidateAsync(DepartmentDTO entity, HttpMethod httpMethod)
        {
        ...
        private List<(int, string)> ValidateRole(List<DepartmentDTO> users, DepartmentDTO entity, string action)
        {
            ...
            if (action == "add")
            {
                ...
                //    errorList.Add((StatusCodes.Status400BadRequest, "Unable to add the role. Duplicate record.")); // firstordefault(where statement as lambda) -> checks if email address already exists from d (all users) to entity (currently handled record) || // if returns true IF inserting data email exists

                if (users.FirstOrDefault(d => d.DepartmentName.ToUpper() == entity.DepartmentName.ToUpper() && !d.IsEnabled) != null)
            ...
            if (action == "edit")
            {
                if (users.FirstOrDefault(d => d.DepartmentID != entity.DepartmentID
                                              && d.DepartmentName.ToUpper() == entity.DepartmentName.ToUpper()) != null)
            ...
            if (action == "delete")
            {
                var userRole = users.Where(x => x.DepartmentID == entity.DepartmentID).FirstOrDefault();
